    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <h1 className="font-display text-4xl font-bold text-foreground mb-2">Final Results</h1>
        ...
        <div className="space-y-3 mb-10">
          {sorted.map((player, i) => (
            <motion.div
              ...
              className={`flex items-center gap-4 p-4 rounded-xl border ${i === 0 ? "gradient-primary text-primary-foreground border-transparent shadow-elevated" : "bg-card border-border shadow-card"}`}
            >
              <span className={`text-2xl font-display font-bold ${i === 0 ? "text-amber-300" : "text-muted-foreground"}`}>
                {i === 0 ? "👑" : `#${i + 1}`}
              </span>
              ...
              <span className={`font-body font-bold text-xl ${i === 0 ? "text-amber-300" : "text-accent"}`}>{player.score}</span>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-display text-2xl font-bold text-foreground">Women You Discovered</h2>
          </div>
            ...
            {state.usedWords.map((entry, i) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <span className="text-2xl">{categoryEmojis[entry.category] || "✨"}</span>
                  <div>
        ...
        <div className="flex gap-3 mt-8 justify-center">
          <button onClick={handleShare} className="gradient-gold text-accent-foreground font-body font-bold py-3 px-6 rounded-xl hover:opacity-90 transition-opacity flex items-center gap-2">
            <Share2 size={18} /> Share Results
          </button>
          <button onClick={() => dispatch({ type: "RESET" })} className="bg-secondary text-secondary-foreground font-body font-bold py-3 px-6 rounded-xl hover:bg-secondary/80 transition-colors flex items-center gap-2">
